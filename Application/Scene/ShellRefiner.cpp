...
namespace Nome::Scene
{
	...
	void CShellRefiner::Refine(float shellHeight, float shellWidth) { 
        ...
        for (int i = 0; i < vertSize; i++)
        {
            ...
            generateNewVertices(vertex, shellWidth);
        }
    ...
    void CShellRefiner::closeFace(Face* face)
    {
        ...
        for (size_t i = 0; i < idxList.size(); i++)
        {
            ...
            std::vector<Edge*> boundaryEdgeList1 =
                currMesh
                    .boundaryEdgeList(); // Randy added this. needed to check if vert is in boundary
	...
	void CShellRefiner::generateNewFaces(Face* face, bool needGrid, bool needShell) {
        ...
        for (size_t i = 0; i < idxList.size(); i++)
        {
            ...
            Face* shellFaceTop = currMesh.addFace(
                { vertex1TopInside, vertex1Top, vertex2Top, vertex2TopInside });
            ...
            if (needShell)
            {
                ...
                Face* shellFaceBotTop =
                    currMesh.addFace({ vertex1CurrInside, vertex1TopInside, vertex2TopInside,
                                        vertex2CurrInside });
                shellFaces.push_back(shellFaceBotTop);

                Face* shellFaceBot = currMesh.addFace(
                    { vertex1Curr, vertex1CurrInside, vertex2CurrInside,
